...
var (
	userManagementInstance *UserManager
	initOnce               sync.Once
)

// GetUserManagerSafe returns the singleton instance of UserManager. Panics if not initialized.
func GetUserManagerSafe() *UserManager {
	if userManagementInstance == nil {
		panic("UserManager not initialized. Call SetUserManager first.")
	}
	return userManagementInstance
}

// SetUserManager initializes the singleton UserManager. Subsequent calls have no effect.
func SetUserManager(um *UserManager) {
	initOnce.Do(func() {
		userManagementInstance = um
	})
}
...
type User struct {
	...
	TrafficSend       int64 // atomic
	TrafficRecv       int64 // atomic
	AlterId           int
	Protocols         []string
	Tags              []string
	CreatedAt         int64
...
type UserManager struct {
	...
	Users         map[int]*User
	mu            sync.Mutex // guards IP operations when needed
}
...
func (um *UserManager) UpdateTrafficUsage(id int, count int64, direction bool) {
	// Ensure the UserManager singleton is initialized
	if um == nil {
		return
	}
	...
	if user != nil && count > 0 {
		user.mu.Lock()
		if direction {
			if &user.TrafficRecv != nil {
				atomic.AddInt64(&user.TrafficRecv, count)
			}
		} else {
			if &user.TrafficSend != nil {
				atomic.AddInt64(&user.TrafficSend, count)
			}
		}
		user.mu.Unlock()
	}
